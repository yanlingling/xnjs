...
/**
 * 只获取工作区的自己的名称，不包含父工作区
 * */
og.getWorkspaceShortName = function (id, includeCurrent) {
    var tree = Ext.getCmp('workspaces-tree');
    var node = tree.tree.getNodeById('ws' + id);
    return node.ws.name;
}
 ...
 * 只获取工作区的自己的名称，不包含父工作区
 * */
og.getWorkspaceScore = function (id, includeCurrent) {
    var tree = Ext.getCmp('workspaces-tree');
    var node = tree.tree.getNodeById('ws' + id);
    return node.ws.score;
}
...
/**
 * 判断是否是最外层的工作区，没有父工作区了
 * @param wsId
 */
og.isTopWorkspace = function (wsId) {
    var tree = Ext.getCmp('workspaces-tree');
    var node = tree.tree.getNodeById('ws' + wsId);
    if (node != null && node.ws.id != 0) {
        // top ws的父节点是ws0根节点
        return node.parentNode.id =='ws0' ? true : false;
    }
}
