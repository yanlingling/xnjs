 ...
 */
ogTasks.drawEditTaskForm = function (task_id, group_id, readOnly) {
 ...
 * @param data
 */
ogTasks.drawDelayApplyForm = function (container_id, data) {
    og.closeTaskDelayApply();
    var sb = new StringBuffer();
    sb.append('<div>延期原因：<textarea id="delay-reason"></textarea>'
        + '<div>申请延期<input id="delay-day"/></div>'

        + '<input type="button" value="提交" onclick="og.submitTaskDelayApply(' + data.taskId + ')">'
        + '<input type="button" value="关闭 " onclick=" og.submitTaskDelayApply(' + data.taskId + ') "/>'
        + '</div>');

    var div = document.createElement('div');
    div.className = 'ogTasksTaskRow';
    div.id = 'ogTaskDelayPanelAT';
...
ogTasks.drawTaskForm = function (container_id, data) {
    ...
    html += "<input  readonly='" + readOnly + "' id='ogTasksPanelATTitle' type='text'"
        + " class='title' name='task[title]' tabIndex=1000 value='' "
        + "onkeypress='return ogTasks.checkEnterPress(event," + data.taskId + ");' />";


    //First column
    html += "<table style='width:100%; margin-top:7px'><tr><td>";
    html += "<div id='ogTasksPanelATDesc' style='display:none'><b>" + lang('description') + ":</b><br/>";
    html += "<textarea  readonly='" + readOnly + "' id='ogTasksPanelATDescCtl' cols='40' rows='10' name='task[text]' class='short' tabIndex=1100 style='height:50px'></textarea></div>";

    var chkIsVisible = data.assignedTo && data.assignedTo.split(':')[1] != '0';
    var chkIsChecked = chkIsVisible && ogTasks.userPreferences.defaultNotifyValue
        && data.assignedTo != (this.currentUser.companyId + ':' + this.currentUser.id);

    html += "<table style='display:none'><tr><td><div id='ogTasksPanelATAssigned' style='padding-top:5px;'><table><tr><td style='width:120px;'><b>"
        + lang('assigned to') + ":&nbsp;</b></td><td><span id='ogTasksPanelATAssignedCont'></span></td></tr></table></div></td>";
    html += '<td style="' + (!data.isEdit ? 'padding-top:7px;' : '') + 'padding-left:15px">';
    html += '<div style="display:' + (chkIsVisible ? 'inline' : 'none')
        + '" id="ogTasksPanelATNotifyDiv"><label for="ogTasksPanelATNotify"><input  readonly="'
        + readOnly + '" style="width:14px;" type="checkbox" name="task[notify]" id="ogTasksPanelATNotify" '
        + (chkIsChecked ? 'checked' : '') + '/>&nbsp;' + lang('send notification') + '</label></div>';
    if (data.isEdit && data.subtasksCount > 0)
        html += '<label for="ogTasksPanelApplyAssignee">'
            + '<input style="width:14px;" type="checkbox" name="task[apply_assignee_subtasks]" id="ogTasksPanelApplyAssignee" />&nbsp;'
            + lang('apply assignee to subtasks') + '</label>';
    html += '</td>';
    html += '</tr></table>';

    html += "<div id='ogTasksPanelATWorkspace' style='display:none;padding-top:5px;"
        + (data.isEdit ? '' : 'display:none') + "'><table><tr><td style='width:120px;'><b>"
        + '科室' + ":&nbsp;</b></td><td><div id='ogTasksPanelWsSelector'></div></td>";
    if (data.isEdit && data.subtasksCount > 0) html += "<td style=\"padding-left:15px\"><label for=\"ogTasksPanelApplyWS\">"
        + "<input readonly =\'" + readOnly + "\' style=\"width:14px;\" type=\"checkbox\" name=\"task[apply_ws_subtasks]\" id=\"ogTasksPanelApplyWS\" />&nbsp;"
        + lang('apply workspace to subtasks') + "</label></td>";
    html += "</tr></table></div>";
    html += "<div id='ogTasksPanelATMilestone' style='display:none;padding-top:5px;" + (data.isEdit ? '' : 'display:none')
        + "'><table><tr><td style='width:120px;'><b>" + lang('milestone') + ":&nbsp;</b></td><td>"
        + "<div id='ogTasksPanelMilestoneSelector'></div></td>";
    if (data.isEdit && data.subtasksCount > 0)
        html += "<td style=\"padding-left:15px\"><label for=\"ogTasksPanelApplyMI\">"
            + "<input style=\"width:14px;\" type=\"checkbox\" name=\"task[apply_milestone_subtasks]\" id=\"ogTasksPanelApplyMI\" />&nbsp;"
            + lang('apply milestone to subtasks') + "</label></td>";
    html += "</tr></table></div>";
    html += "<div style='display: none' id='ogTasksPanelATTags' style='padding-top:5px;"
        + (data.isEdit ? '' : 'display:none') + "'><table><tr><td style='width:120px;'><b>"
        + lang('tags') + ":&nbsp;</b></td><td>"
        + "<input id='ogTasksPanelTagsSelector' style='min-width:120px;max-width:300px' type='text' value='"
        + (data.tags ? data.tags + ',' : '') + "' name='task[tags]'/></td></tr></table></div>";
    html += "<div id='ogTasksPanelATObjectType' style='padding-top:5px;display: none'><table><tr><td style='width:120px;'><b>"
        + lang('object type') + ":&nbsp;</b></td><td>"
        + "<input readonly =\'" + readOnly + "\' id='ogTasksPanelObjectTypeSelector' style='min-width:120px;max-width:300px' type='text' "
        + "value='" + (data.otype ? data.otype : og.defaultTaskType) + "' name='task[object_subtype]'/></td></tr></table></div>";

    //Second column
    html += "</td><td style='padding-left:10px; margin-right:10px;width:300px;'>";

    if (drawOptions.show_time) {
        html += "<div id='ogTasksPanelATTime' style='padding-top:5px;'><table><tr><td style='width:120px;'><b>"
            + lang('time worked') + ":</b></td><td>";
        html += "<input type='text' readonly =\'" + readOnly + "\' id='ogTasksPanelATHours' style='width:25px' tabIndex=1250 />&nbsp;"
            + lang('hours') + "</td></tr></table></div>";
    }

    html += "<table id='ogTasksPanelATDates' style='padding-top:5px;" + (data.isEdit ? '' : 'display:none')
        + "'><tr style='display: none' ><td style='width:120px;'><b>" + lang('start date') + ":</b>&nbsp;</td>";
    ...
    html += "<div id='ogTasksPanelATPriority' style='padding-top:5px;" + (data.isEdit ? '' : 'display:none')
        + "'><table><tr><td style='width:120px;'><b>" + lang('priority') + ":&nbsp;</b></td>";
    ...
    html += "<a href='#' class='internalLink' style='" + (data.isEdit ? '' : 'display:none')
        + "' onclick='ogTasks.TaskFormShowAll(" + data.taskId + ")' id='ogTasksPanelATShowAll'><b>"
        + lang('all options') + "...</b></a>";
    html += "</td><td align=right>";

    if (readOnly != true) {
        html += "<button onclick='ogTasks.SubmitNewTask(" + data.taskId
            + ");return false;' tabIndex=1600 type='submit' class='submit'>"
            + (data.isEdit ? lang('save changes') : lang('add task'))
            + "</button>&nbsp;&nbsp;";
    }
    //Buttons
    html += " <button tabIndex=1700 onclick='ogTasks.hideAddNewTaskForm();return false;'>"
        + lang('cancel') + "</button>";
    html += "</td></table>";
...
og.closeTaskDelayApply = function () {
    var oldForm = document.getElementById('ogTaskDelayPanelAT');
    if (oldForm) {
        oldForm.parentNode.removeChild(oldForm);
    }
}
