...
var rx__TasksDrag = {
    ...
    process: function () {
        ...
        switch (this.displayCriteria.group_by) {
            case 'status':
                ogTasks.ToggleCompleteStatus(task.id, 1 - this.d);
                return;
                break;
            default:
        }
        ...
        switch (this.displayCriteria.group_by) {
            case 'milestone':
                parameters["milestone_id"] = this.d != 'unclassified' ? ogTasks.getMilestone(this.d).id : 0;
                break;
            case 'priority':
                parameters["priority"] = this.d != 'unclassified' ? parseInt(this.d) : 200;
                /*100,200,300*/
                break;
            case 'assigned_to':
                parameters["assigned_to"] = this.d;
                /* 1:1 */
                break;
            case 'due_date' :
                if (group_not_empty) parameters["task_due_date"] = group.group_tasks[0].dueDate;
                break;
            case 'start_date' :
                if (group_not_empty) parameters["task_start_date"] = group.group_tasks[0].startDate;
                break;
            case 'created_on' :
                if (group_not_empty) parameters["created_on"] = group.group_tasks[0].createdOn;
                break;
            case 'completed_on':
                if (group_not_empty) parameters["completed_on"] = group.group_tasks[0].completedOn.toString().format(lang('date format'));
                break;
            case 'created_by' :
                parameters["created_by"] = this.d;
                /* ? */
                break;
            case 'status' :
                parameters["status"] = this.d;
                /* done previously, special request */
                break;
            case 'completed_by':
                parameters["completed_by"] = this.d;
                /* ? */
                break;
            case 'subtype':
                parameters["object_subtype"] = this.d;
                /* ? */
                break;
            case 'tag':
            {
...
ogTasks.draw = function () {
    ...
    //局长的话 以科室（工作区为单位展示）
    if (this.userRole == USER_ROLE_JUZHANG) {
        sb.append(this.drawGroupWithoutTaskDetail());
    } else {
        for (var i = 0; i < this.Groups.length; i++) {
...
ogTasks.drawGroupWithoutTaskDetail = function () {
    var sb = new StringBuffer();
    sb.append("<table width='100%'>");
    sb.append("<tr><td>科室名称</td><td>效能状态</td><td>效能考核</td><td>处理延期申请</td></tr>");
    for (var i = 0, item; item = this.Groups[i++];) {
        if (item.group_name == '未分组的') {
            continue;
        }
        if (i % 2 == 0) {
            sb.append('<tr class="dashAltRow">');
        } else {
            sb.append('<tr >');
        }
        sb.append('<td><span style="display: none" id="task-ws-' + item.group_id + '"></span>' + og.clean(item.group_name) + '</td>');
        sb.append('<td>' + item.score + '</td>');
        sb.append('<td>' + this.getWsLightHTML(item) + '</td>');
        sb.append('</tr>');
    }
    sb.append('</table>');
    return sb.toString();
 ...
 * @param wsData
 */
ogTasks.getWsLightHTML = function (wsData) {
    var tasks = wsData.group_tasks;
    var red = [];
    var yellow = [];
    var gray = [];
    var green = [];
    if (tasks.length == 0) {
        return this.buildLightHtml('green');
    }
    for (var i = 0, item; item = tasks[i++];) {
        var status = this.getTaskStatus(item);
        switch (status) {
            case 'red':
                red.push(item.id);
                break;
    ...
    if (red.length != 0) {
        return this.buildLightHtml('red');
    } else if (yellow.length != 0) {
        return this.buildLightHtml('yellow');
    } else if (gray.length != 0) {
        return this.buildLightHtml('gray');
    }
    return this.buildLightHtml('green');
}
 ...
 * @returns {string}
 */
ogTasks.buildLightHtml = function (color) {
    return '<div class="db-ico ico-task-light-' + color + '"></div>';
}
 ...
 * @param task
 */
ogTasks.getTaskStatus = function (task) {
    var dueDate = new Date(task.dueDate * 1000).getTime();
    var now = new Date().getTime();
    var minus = now - dueDate;
    var dayMillSec = 24 * 60 * 60 * 1000;
    var days = Math.ceil(minus / dayMillSec);
    // 1为完成状态
    if (task.status == 1) {
        return 'green';
    } else {
 ...
 * @returns {*}
 */
ogTasks.drawTaskOpt = function (task, group_id) {
    var sb = new StringBuffer();
    sb.append("<a href='#' onclick='ogTasks.drawEditTaskForm(" + task.id + ", \"" + group_id + "\",true)'>");
    sb.append("" + lang('edit') + "</a>");
    // 未完成的时候，显示完成操作
    if (task.status == 0) {
        sb.append("&nbsp;&nbsp;<a href='#' onclick='ogTasks.ToggleCompleteStatus(" + task.id + ", " + task.status + ")'>");
        sb.append(lang('do complete') + "</a>");
        sb.append("&nbsp;&nbsp;<a href='#' onclick='ogTasks.drawDelayApply(" + task.id  + ", " + group_id + ")'>");
        sb.append('延期申请' + "</a>");
    }
    return sb.toString();
}
ogTasks.drawGroup = function (displayCriteria, drawOptions, group) {
    ...
    sb.append("<tr><td  class=\"tasktd1\">任务</td><td class=\"tasktd2\">责任人</td>"
        +"<td class=\"tasktd3\">截止时间</td><td class=\"tasktd4\">效能状态</td>"
        +"<td class=\"tasktd5\">操作</td></tr></table></div>");
    var tasks = group.group_tasks;
    var group_id = group.group_id;

    for (var i = 0, task; task = tasks[i++];) {
        var containerName = 'ogTasksPanelTask' + task.id + 'G' + group_id;
        var tgId = "T" + task.id + 'G' + group_id;
        sb.append('<div id="' + containerName + '"><table width="100%">');
        if (i % 2 == 0) {
            sb.append('<tr class="dashAltRow">');
        } else {
            sb.append('<tr >');
        }
        sb.append('<td  class=\"tasktd1\"><span style="display: none" id="' + tgId + '"></span>'
            + og.clean(task.title) + '</td>');
        sb.append('<td class=\"tasktd2\">' + og.clean(this.getUserCompanyName(task.assignedToId)) + '</td>');
        ...
        sb.append('<td class=\"tasktd3\">' + dateFormatted + '</td>');
        var status = this.getTaskStatus(task);
        sb.append('<td class=\"tasktd4\">' + this.buildLightHtml(status) + '</td>');
        sb.append('<td class=\"tasktd5\">' + this.drawTaskOpt(task, group_id) + '</td>');
        sb.append('</tr>');
        sb.append('</table></div>');
    }
    return sb.toString();
...
ogTasks.drawTaskRow = function (task, drawOptions, displayCriteria, group_id, level) {
    var sb = new StringBuffer();
    var tgId = "T" + task.id + 'G' + group_id;
    sb.append('<table id="ogTasksPanelTaskTable' + tgId +
        '" class="ogTasksTaskTable' + (task.isChecked ? 'Selected' : '')
        + '" onmouseover="ogTasks.mouseMovement(' + task.id + ',\'' + group_id
        + '\',true)" onmouseout="ogTasks.mouseMovement(' + task.id + ',\'' + group_id + '\',false)"><tr>');
    ...
    if (task.subtasks.length > 0) {
        sb.append("<td width='16px' style='padding-top:3px'><div id='ogTasksPanelFixedExpander" + tgId
            + "' class='og-task-expander " + ((task.isExpanded) ? 'toggle_expanded' : 'toggle_collapsed')
            + "' onclick='ogTasks.toggleSubtasks(" + task.id + ", \"" + group_id + "\")'></div></td>");
    } else {
        sb.append("<td width='16px'><div id='ogTasksPanelExpander"
            + tgId + "' style='visibility:hidden;margin-top:2px;' " +
            "class='og-task-expander ico-add ogTasksIcon' onClick='ogTasks.drawAddNewTaskForm(\""
            + group_id + "\", " + task.id + "," + level + ")' title='" + lang('add subtask') + "'></div></td>");
    }

    if (task.isRead) {
        sb.append("<td style=\"width:16px\" id=\"ogTasksPanelMarkasTd"
            + task.id + "\"><div title=\"" + lang('mark as unread') + "\" id=\"readunreadtask"
            + task.id + "\" class=\"db-ico ico-read\" onclick=\"ogTasks.readTask(" + task.id + "," + task.isRead + ")\" /></td>");
    } else {
        sb.append("<td style=\"width:16px\" id=\"ogTasksPanelMarkasTd"
            + task.id + "\"><div title=\"" + lang('mark as read')
            + "\" id=\"readunreadtask" + task.id + "\" class=\"db-ico ico-unread\" onclick=\"ogTasks.readTask("
            + task.id + "," + task.isRead + ")\" /> </td>");
    }
    ...
    sb.append('<a class="internalLink" href="' + viewUrl
        + '" onclick="og.openLink(\'' + viewUrl + '\');return false;" id="rx__dd' + (++rx__dd) + '">' + taskName + '</a>');
    ...
    if (task.status > 0) {
        sb.append("<div class='ico-reopen coViewAction' title='"
            + lang('reopen this task') + "' style='cursor:pointer;height:16px;padding-top:0px'>"
            + lang('reopen') + "</div></a></td>");
    } else {
        sb.append("<div class='ico-complete coViewAction' title='"
            + lang('complete this task') + "' style='cursor:pointer;height:16px;padding-top:0px'>"
            + lang('do complete') + "</div></a></td>");
    }
...
ogTasks.drawDelayApply= function (task_id, group_id) {
    var task = this.getTask(task_id);
    var containerName = 'ogTasksPanelTask' + task.id + 'G' + group_id;
    if (task) {
        this.drawDelayApplyForm(containerName, {
            taskId: task.id
        });
    }
}
