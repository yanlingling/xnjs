...
ogTasks.loadDataFromHF = function(){
    ...
    result['userRole'] = Ext.util.JSON.decode(document.getElementById('hfUserRole').value);
...
ogTasks.getGroupData = function(displayCriteria, groups,tasks){
	...
	for (var i = 0; i < groups.length; i++){
		...
		if (groupId != 'unclassified'){
            score = og.getWorkspaceScore(groupId, true);
			switch(displayCriteria.group_by){
				case 'milestone':
					icon = 'ico-milestone';
				...
				case 'workspace' : 
					...
					name = og.getWorkspaceShortName(groupId, true);
					break;
				case 'assigned_to' : 
...
ogTasks.groupTasks = function(displayCriteria, tasksContainer){
    ...
    displayCriteria.group_by = 'workspace';  //  强制按工作区分组
	if (!this.redrawGroups)
	...
	for (var i = 0; i < tasksContainer.length; i++){
		...
		if (!task.parent){
            ...
            // 默认全是按workspace分组，以前各种分组条件被我删掉了
            if (ids && ids.length > 0)
            {
