...
/* 声明游标  */
DECLARE rs CURSOR  FOR  SELECT id, due_date,light_status, assigned_to_departid  FROM  og_project_tasks where deleted!=1 and light_status!=0;

/* 声明游标  */
DECLARE rs1 CURSOR  FOR  SELECT yellow_to_minus, yellow_had_minus  FROM  og_department where depart_id=assigned_to_departid1;
       ...
       /*超过1天的时候，变黄灯灯,三个扣2分*/
        ...
        IF DATEDIFF (CURRENT_DATE ( ) ,DATE(due_date1) ) =1   THEN
             if  light_status1 !=3 then
                    ...
                    /*yellow_had_minus用来记录还没扣分的任务id*/
                UPDATE  og_department SET yellow_to_minus =yellow_to_minus+1,yellow_had_minus =CONCAT(yellow_had_minus,',',id1)  WHERE  depart_id = assigned_to_departid1;
                    /* 打开游标 */
                OPEN rs1;
                ...
                IF  NOT Done THEN
                        ...
                        if yellow_task_num =3 then
                              ...
                              UPDATE  og_department SET score =score-2 ,yellow_to_minus=0,yellow_had_minus='' WHERE  depart_id = assigned_to_departid1;
                         end if;
                   end if;
                    FETCH  NEXT  FROM  rs1  INTO yellow_task_num, yellow_task;
