...
foreach ($tasks_in_progress as $task) {
	...
	?>
		<tr class="<?php echo $c % 2 == 1? '':'dashAltRow'?>">
            <td><div class="db-ico ico-task<?php echo $task->getRemainingDays() > 7? '-light-green' : ($task->getRemainingDays()>0?'-light-yellow' : '-light-red') ?>">&nbsp;></div></td>
	...
	<a class='internalLink' href='<?php echo $task->getViewUrl() ?>'>
        <?php echo clean($task->getTitle())?><?php echo $text ?></a>
            </td>
