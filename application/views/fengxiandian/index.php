...
<div class="content-wraper" id="my-risk-tab-content">
    <div class="task-bulletin">
        <table width="100%" border="0" class="og-table" style="text-align: center">
            ...
            <tr>
                <td id="risk-light-count-4">0</td>
                <td id="risk-light-count-3">0</td>
                <td id="risk-light-count-2">0</td>
                <td id="risk-light-count-1">0</td>
                <td id="risk-person-score"></td>
            </tr>
    ...
    <div id="createLearningBtn" style="margin-top: 10px" class="<?php echo $role == '科长' ? '' : 'hide' ?>">
        <span class="new-button" id='view-by-depart'
              onclick="og.risk.goToDepartLearning(<?php echo $risk_list[1]['depart_id'] ?>)">我的科室</span>
    </div>
...
<div id="risk-content-tab-content" class="hide">
    <div id="createLearningBtn" style="margin-top: 10px" class="<?php echo $canCreateRisk == 0 ? 'hide' : '' ?>">
        <span class="new-button " id='add-text-risk' onclick="og.risk.addPersonQuestion()">新建个人问卷</span>
        <span class="new-button" id='add-vedio-risk' onclick="og.risk.addDepartQuestion()">新建科室问卷</span>
    </div>
    <div class="table-header">
        <table width='100%' class="og-table">
            <tr>
...
function getRiskOptContent($status, $riskId, $contentId, $isSelf)
{
    ...
    if (!$isSelf) {
        ...
        if ($status == 1) {
            ...
            return "<a onclick=og.risk.toRiskContent($riskId,$contentId,'learn')>开始自查</a>";
        }
    }
...
<script>
    function renderBulletin() {
        ...
        if (riskOverviewData) {
            ...
            for (var i = 0, item; item = riskOverviewData[i++];) {
                ...
                $('#risk-light-count-' + status).html(item['light_count']);
            }
            $('#risk-person-score').html(baseScore);
        }
    }
    renderBulletin();
    ...
    function showSubTab(ele) {
        ...
        if ($.trim(ele.html()) == '个人风险点') {
            $('#my-risk-tab-content').removeClass('hide');
        } else if ($.trim(ele.html()) == '科室风险点') {
