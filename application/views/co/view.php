		...
		if (count(PageActions::instance()->getActions()) > 0 ) { ?>
			<div id="actionsDialog1"> <?php
				...
				foreach ($pactions as $action) {
                    if($action->getTitle() == '查看历史记录'||
                        $action->getTitle() == '更新肖像'||
                        $action->getTitle() == '更新信息'||
                        $action->getTitle() == '编辑属性'){
                        continue;
                    }
					if ($action->isCommon) {
				 		...
				 		<?php } else { ?>
							<a id="<?php $atrib = $action->getAttributes(); echo array_var($atrib,'id'); ?>"
                               style="display:block"
                               class="<?php $attribs = $action->getAttributes(); echo isset($attribs["download"]) ? '':'internalLink' ?> coViewAction <?php echo $action->getName()?>" href="<?php echo $action->getURL()?>"> <?php echo $action->getTitle(); ?></a>
						<?php }
				 		$shown++;
					} //if
				}//foreach ?>
...
<!-- Properties Panel -->
<table style="width:240px;display: none">
	<col width=12/><col width=216/><col width=12/>
	<tr>
