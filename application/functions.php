...
function getCommonSuperviseStatus($status)
{
    $str = '';
    switch ($status) {
        case 0:
...
function getDateFromSql($dateStr)
{
...
{
    return (strtotime(getDateFromSql($dateStr)) - strtotime(date('y-m-d', time()))) / 86400;
}

function getDateLong($dateStr1, $dateStr2)
{
    return ((strtotime($dateStr2) - strtotime($dateStr1)) / 86400) + 1;
}
...
function sendShortMessageToDriver($departName, $beginTime, $endTime, $carNumber, $phone)
{
    $client = new SoapClient('http://202.102.41.99:8090/wsewebsm/services/SendMessageService?wsdl');
    $content = $departName . "于" . $beginTime . "至" . $endTime . "预约使用" . $carNumber . "的车辆，请提前做好准备，如有问题请提前与办公室联系。";
...
{
    $client = new SoapClient('http://202.102.41.99:8090/wsewebsm/services/SendMessageService?wsdl');
    $content = $departName . "于" . $beginTime . "至" . $endTime . "预约用车， 请处理。";
    // 办公室3月20日 3点-4点预约使用苏G的车辆，请提前做好准备，如有问题请提前与办公室联系。
    $res = $client->sendSms(array('username' => '0518C00121789', 'password' => 'gyyjj2013'),
        array('content' => $content, 'receiveNum' => $phone, 'sendType' => '1',
            'signature' => '赣榆县食品药品监督管理局'));
...
function sendShortMessageToKezhang($beginTime, $endTime, $phone, $carNumber, $driver_name, $driver_phone)
{
    ...
    $client = new SoapClient('http://202.102.41.99:8090/wsewebsm/services/SendMessageService?wsdl');
    $content = "您" . $beginTime . "至" . $endTime . "的用车申请已被批准。车牌：" . $carNumber . "，司机：" . $driver_name . "，电话：" . $driver_phone . "。";
    $res = $client->sendSms(array('username' => '0518C00121789', 'password' => 'gyyjj2013'),
        array('content' => $content, 'receiveNum' => $phone, 'sendType' => '1',
            'signature' => '赣榆县食品药品监督管理局'));
    writeMessageLog($res);
...
function writeMessageLog($res)
{
    $res0 = $res;
    $res = json_encode($res);

    $fileName = date('Y-m-d') . '.log';
    writeFile($fileName, $res, 'a');
}
